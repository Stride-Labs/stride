...
echo $GAIA_VAL_KEY | $GAIA_CMD keys add $GAIA_VAL_ACCT --recover --keyring-backend=test > /dev/null
...
# Add relayer account
echo $RLY_MNEMONIC_2 | $GAIA_CMD keys add rly2 --recover --keyring-backend=test > /dev/null
...
sed -i -E "s|26657|26667|g" "${STATE}/${GAIA_NODE_NAME}/config/client.toml"
sed -i -E "s|26656|26666|g" "${STATE}/${GAIA_NODE_NAME}/config/config.toml"
sed -i -E "s|26657|26667|g" "${STATE}/${GAIA_NODE_NAME}/config/config.toml"
sed -i -E "s|26658|26668|g" "${STATE}/${GAIA_NODE_NAME}/config/config.toml"
sed -i -E "s|26660|26670|g" "${STATE}/${GAIA_NODE_NAME}/config/config.toml"
