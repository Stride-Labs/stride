...
echo $STRIDE_VAL_KEY | $STRIDE_CMD keys add $STRIDE_VAL_ACCT --recover --keyring-backend=test > /dev/null
# get validator address
val_addr=$($STRIDE_CMD keys show $STRIDE_VAL_ACCT --keyring-backend test -a)
...
# Add relayer account
echo $RLY_MNEMONIC_1 | $STRIDE_CMD keys add rly1 --recover --keyring-backend=test > /dev/null
