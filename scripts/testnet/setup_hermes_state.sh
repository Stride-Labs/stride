...
sed -i -E "s|STRIDE_CHAIN_ID|$STRIDE_CHAIN_ID|g" $HERMES_STARTUP_FILE
sed -i -E "s|STRIDE_ENDPOINT|$STRIDE_MAIN_ENDPOINT|g" $HERMES_STARTUP_FILE

HERMES_KEYS_FILE="${STATE}/hermes_keys.sh"
cp ${SCRIPT_DIR}/hermes_keys_template.sh $HERMES_KEYS_FILE
sed -i -E "s|STRIDE_CHAIN_ID|$STRIDE_CHAIN_ID|g" $HERMES_KEYS_FILE
sed -i -E "s|HERMES_STRIDE_MNEMONIC|$HERMES_STRIDE_MNEMONIC|g" $HERMES_KEYS_FILE
sed -i -E "s|HERMES_GAIA_MNEMONIC|$HERMES_GAIA_MNEMONIC|g" $HERMES_KEYS_FILE

rm -f "${HERMES_CONFIG_FILE}-e"
rm -f "${HERMES_STARTUP_FILE}-e"
rm -f "${HERMES_KEYS_FILE}-e"
