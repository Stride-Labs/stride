...
HOST_BINARY tx ibc-transfer transfer transfer $transfer_channel stride1u20df3trc2c2zdhm8qvh2hdjx9ewh00sv6eyy8 4000000HOST_DENOM --from hot --chain-id HOST_CHAIN_ID -y --keyring-backend test --node HOST_RPC --fees 150000HOST_DENOM
...
build/strided --home STRIDE_HOME tx stakeibc add-validators HOST_CHAIN_ID validator.json --gas 1000000 --chain-id STRIDE_CHAIN_ID --keyring-backend test --from admin -y
rm validator.json
...
echo '{"validators": [{"name": "HOST_VAL_ADDRESS_2", "address": "HOST_VAL_NAME_2", "weight": 10}]}' > validator.json
build/strided --home STRIDE_HOME tx stakeibc add-validators HOST_CHAIN_ID validator.json --gas 1000000 --chain-id STRIDE_CHAIN_ID --keyring-backend test --from admin -y
rm validator.json
