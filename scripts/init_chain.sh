...
VAL_MNEMONICS_VAR=${CHAIN}_VAL_MNEMONICS
IFS=',' read -r -a VAL_MNEMONICS <<< "${!VAL_MNEMONICS_VAR}"

HERMES_ACCT=$(GET_VAR_VALUE      HERMES_${CHAIN}_ACCT)
HERMES_MNEMONIC=$(GET_VAR_VALUE  HERMES_${CHAIN}_MNEMONIC)
...
REV_MNEMONIC_VAR=${CHAIN}_REV_MNEMONIC
...
for (( i=1; i <= $NUM_NODES; i++ )); do
    ...
    if [[ "$CHAIN" == "INJECTIVE" ]]; then
        cmd="docker-compose run --rm injective${i} injectived"
    fi
...
HERMES_ADDRESS=$($MAIN_NODE_CMD keys show $HERMES_ACCT --keyring-backend test -a | tr -cd '[:alnum:]._-')
...
if [ "$CHAIN_ID" == "$STRIDE_CHAIN_ID" ]; then 
    ...
    STRIDE_ADMIN_ADDRESS=$($MAIN_NODE_CMD keys show $STRIDE_ADMIN_ACCT --keyring-backend test -a | tr -cd '[:alnum:]._-')
