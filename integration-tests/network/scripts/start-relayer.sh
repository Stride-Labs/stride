...
create_path() {
    # If there aren't any channels yet, create a new path
    if ! rly q channels $CHAIN_NAME_A 2>/dev/null | grep -q $CHAIN_ID_A; then
        echo "Creating path..."
        rly tx link $PATH_NAME
    else
        # Otherwise, add the exising connection to the config
