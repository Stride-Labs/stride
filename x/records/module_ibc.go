...
func (im IBCModule) OnAcknowledgementPacket(
	...
	case *channeltypes.Acknowledgement_Result:
		successLog := fmt.Sprintf("\t [IBC-TRANSFER] Acknowledgement_Result {%s}", string(resp.Result))
		im.keeper.Logger(ctx).Error(successLog)
		// UPDATE RECORD
		// match record based on amount
		// update the record
	case *channeltypes.Acknowledgement_Error:
		errorLog := fmt.Sprintf("\t [IBC-TRANSFER] Acknowledgement_Error {%s}", resp.Error)
		im.keeper.Logger(ctx).Error(errorLog)
	}
