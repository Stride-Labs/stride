...
func (s *KeeperTestSuite) TestEpochInfoChangesBeginBlockerAndInitGenesis() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fn: func() {
				ctx := s.Ctx.WithBlockHeight(2).WithBlockTime(now.Add(time.Second))
				s.App.EpochsKeeper.BeginBlocker(ctx)
				epochInfo, _ = s.App.EpochsKeeper.GetEpochInfo(ctx, "monthly")
			},
		},
		{
			expCurrentEpochStartHeight: 2,
	...
	for i, tc := range testCases {
		s.Run(fmt.Sprintf("Case %d", i), func() {
			...
			epochInfos := s.App.EpochsKeeper.AllEpochInfos(ctx)
			for _, epochInfo := range epochInfos {
				s.App.EpochsKeeper.DeleteEpochInfo(ctx, epochInfo.Identifier)
			}

			// check init genesis
			ctx = ctx.WithBlockHeight(1).WithBlockTime(now)
			s.App.EpochsKeeper.InitGenesis(ctx, types.GenesisState{
...
func (s *KeeperTestSuite) TestEpochStartingOneMonthAfterInitGenesis() {
	...
	s.Require().Equal(epochInfo.CurrentEpochStartHeight, ctx.BlockHeight())
