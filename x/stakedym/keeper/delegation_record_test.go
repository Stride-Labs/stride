...
func (s *KeeperTestSuite) TestSafelySetDelegationRecord() {
	// Set one record
	err := s.App.StakedymKeeper.SafelySetDelegationRecord(s.Ctx, types.DelegationRecord{Id: 1})
	...
	err = s.App.StakedymKeeper.SafelySetDelegationRecord(s.Ctx, types.DelegationRecord{Id: 2})
...
func (s *KeeperTestSuite) TestGetDelegationRecord() {
	...
	for i := 0; i < len(delegationRecords); i++ {
		...
		actualRecord, found := s.App.StakedymKeeper.GetDelegationRecord(s.Ctx, recordId)
...
func (s *KeeperTestSuite) TestArchiveDelegationRecord() {
	...
	for removedIndex := 0; removedIndex < len(delegationRecords); removedIndex++ {
		...
		s.App.StakedymKeeper.ArchiveDelegationRecord(s.Ctx, removedRecord)

		// Confirm removed from active
		_, found := s.App.StakedymKeeper.GetDelegationRecord(s.Ctx, removedRecord.Id)
		s.Require().False(found, "record %d should have been removed", removedRecord.Id)

		// Confirm placed in archive
		_, found = s.App.StakedymKeeper.GetArchivedDelegationRecord(s.Ctx, removedRecord.Id)
		...
		for checkedIndex := removedIndex + 1; checkedIndex < len(delegationRecords); checkedIndex++ {
			...
			_, found := s.App.StakedymKeeper.GetDelegationRecord(s.Ctx, checkedId)
	...
	archivedRecords := s.App.StakedymKeeper.GetAllArchivedDelegationRecords(s.Ctx)
	for i := 0; i < len(delegationRecords); i++ {
...
func (s *KeeperTestSuite) TestGetAllActiveDelegationRecords() {
	...
	actualRecords := s.App.StakedymKeeper.GetAllActiveDelegationRecords(s.Ctx)
