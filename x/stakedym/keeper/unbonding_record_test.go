...
func (s *KeeperTestSuite) TestGetUnbondingRecord() {
	...
	for i := 0; i < len(unbondingRecords); i++ {
		...
		actualRecord, found := s.App.StakedymKeeper.GetUnbondingRecord(s.Ctx, recordId)
...
func (s *KeeperTestSuite) TestArchiveUnbondingRecord() {
	...
	for removedIndex := 0; removedIndex < len(unbondingRecords); removedIndex++ {
		...
		s.App.StakedymKeeper.ArchiveUnbondingRecord(s.Ctx, removedRecord)

		// Confirm removed from the active store
		_, found := s.App.StakedymKeeper.GetUnbondingRecord(s.Ctx, removedRecord.Id)
		s.Require().False(found, "record %d should have been removed", removedRecord.Id)

		// Confirm moved to the archive store
		_, found = s.App.StakedymKeeper.GetArchivedUnbondingRecord(s.Ctx, removedRecord.Id)
		...
		for checkedIndex := removedIndex + 1; checkedIndex < len(unbondingRecords); checkedIndex++ {
			...
			_, found := s.App.StakedymKeeper.GetUnbondingRecord(s.Ctx, checkedId)
...
func (s *KeeperTestSuite) TestGetAllActiveUnbondingRecords() {
	...
	actualRecords := s.App.StakedymKeeper.GetAllActiveUnbondingRecords(s.Ctx)
...
func (s *KeeperTestSuite) TestGetAccumulatingUnbondingRecord() {
	...
	actualAccumulatingRecord, err := s.App.StakedymKeeper.GetAccumulatingUnbondingRecord(s.Ctx)
	...
	_, err = s.App.StakedymKeeper.GetAccumulatingUnbondingRecord(s.Ctx)
	s.Require().ErrorContains(err, "more than one record")

	// Remove the ACCUMULATING records and confirm it errors
	s.App.StakedymKeeper.RemoveUnbondingRecord(s.Ctx, expectedRecordId)
	s.App.StakedymKeeper.RemoveUnbondingRecord(s.Ctx, duplicateAccumulatingRecordId)
...
func (s *KeeperTestSuite) TestSafelySetUnbondingRecord() {
	...
	s.App.StakedymKeeper.SetUnbondingRecord(s.Ctx, types.UnbondingRecord{Id: 1})

	// Try to set another record at ID 1 using SafelySet - it should error
	err := s.App.StakedymKeeper.SafelySetUnbondingRecord(s.Ctx, types.UnbondingRecord{Id: 1})
	s.Require().ErrorContains(err, "unbonding record already exists")

	// Set a record at ID 2 - it should succeed
	err = s.App.StakedymKeeper.SafelySetUnbondingRecord(s.Ctx, types.UnbondingRecord{Id: 2})
	s.Require().NoError(err, "no error expected when setting new record")

	// Confirm there are two records
	unbondingRecords := s.App.StakedymKeeper.GetAllActiveUnbondingRecords(s.Ctx)
