...
func (keeper Keeper) GetDeposit(ctx sdk.Context, creator sdk.AccAddress, host_chain_id string) (types.Deposit, bool) {
	store := ctx.KVStore(keeper.storeKey)

	bz := store.Get(types.DepositKey(creator.String(), host_chain_id))
	if bz == nil {
...
func (keeper Keeper) DeleteDeposit(ctx sdk.Context, creator string, host_chain_id string) {
	store := ctx.KVStore(keeper.storeKey)

	store.Delete(types.DepositKey(creator, host_chain_id))
}
