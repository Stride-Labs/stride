...
const (
	ICACallbackID_CastVoteOnHost = "castvoteonhost"
)

// ICACallbacks wrapper struct for liquidgov keeper
...
func (c ICACallbacks) RegisterICACallbacks() icacallbackstypes.ICACallbackHandler {
	a := c.
		AddICACallback(ICACallbackID_CastVoteOnHost, ICACallback(CastVoteOnHostCallback))
