...
// Proposal to update an oracle's cosmwasm contract address
func CmdUpdateOracleContractProposal() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-oracle-contract [proposal-file]",
		Short: "Submit a proposal to update an oracle's cosmwasm contract address",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Submit a update-oracle-contract proposal along with an initial deposit.
The proposal details must be supplied via a JSON file.

Example:
$ %s tx gov submit-legacy-proposal update-oracle-contract <path/to/proposal.json> --from=<key_or_address>
Where proposal.json contains:
{
...
`, version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var proposal types.UpdateOracleContractProposal
			if err := parseProposalFile(clientCtx.Codec, proposalFile, &proposal); err != nil {
				return err
			}
