...
type Oracle struct {
	...
	FlushPendingMetrics bool   `protobuf:"varint,8,opt,name=flush_pending_metrics,json=flushPendingMetrics,proto3" json:"flush_pending_metrics,omitempty"`
}
...
func (m *Oracle) GetFlushPendingMetrics() bool {
	if m != nil {
		return m.FlushPendingMetrics
	}
	return false
}
...
func (m *Oracle) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field FlushPendingMetrics", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.FlushPendingMetrics = bool(v != 0)
		default:
			iNdEx = preIndex
