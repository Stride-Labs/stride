...
func (c ICACallbacks) RegisterICACallbacks() icacallbackstypes.ICACallbackHandler {
	a := c.
		AddICACallback(ICACallbackID_InstantiateOracle, ICACallback(InstantiateOracleCallback)).
		AddICACallback(ICACallbackID_UpdateOracle, ICACallback(UpdateOracleCallback))
