...
func (s *KeeperTestSuite) TestClaimCallback_Successful() {
	...
	s.Require().Equal(hzu1.Status, recordtypes.HostZoneUnbonding_STATUS_TRANSFERRED, "hzu1 status set to transferred")
	// verify the other hzus are unchanged
	s.Require().Equal(hzu2.NativeTokenAmount, hzu2.NativeTokenAmount, "hzu2 amount unchanged")
	s.Require().Equal(hzu2.Status, recordtypes.HostZoneUnbonding_STATUS_UNBONDED, "hzu2 status set to transferred")
	s.Require().Equal(hzu3.NativeTokenAmount, hzu3.NativeTokenAmount, "hzu3 amount unchanged")
	s.Require().Equal(hzu3.Status, recordtypes.HostZoneUnbonding_STATUS_UNBONDED, "hzu3 status set to transferred")
	s.Require().Equal(hzu4.NativeTokenAmount, hzu4.NativeTokenAmount, "hzu4 amount unchanged")
	s.Require().Equal(hzu4.Status, recordtypes.HostZoneUnbonding_STATUS_UNBONDED, "hzu4 status set to transferred")
