...
func (k Keeper) DetokenizeCallback(ctx sdk.Context, packet channeltypes.Packet, ackResponse *icacallbackstypes.AcknowledgementResponse, args []byte) error {
	...
	// if StrictUnbondingCap is enabled, then we need to detokenize the shares into native stake
	// and delegate the native stake to the validator
	if k.IsStrictUnbondingEnabled(ctx) {
