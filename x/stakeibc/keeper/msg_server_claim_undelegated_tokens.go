...
func (k msgServer) ClaimUndelegatedTokens(goCtx context.Context, msg *types.MsgClaimUndelegatedTokens) (*types.MsgClaimUndelegatedTokensResponse, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("%s: %s", errMsg, types.ErrRecordNotFound.Error())
	}

	icaTx, err := k.GetRedemptionTransferMsg(ctx, userRedemptionRecord, msg.HostZoneId)
	if err != nil {
		return nil, fmt.Errorf("unable to build redemption transfer message: %s", err.Error())
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to marshal claim callback args: %s", err.Error())
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("unable to submit ICA redemption tx: %s", err.Error())
	}
...
func (k Keeper) GetClaimableRedemptionRecord(ctx sdk.Context, msg *types.MsgClaimUndelegatedTokens) (*recordstypes.UserRedemptionRecord, error) {
	...
	if !found {
		...
		k.Logger(ctx).Error(errMsg)
		return nil, fmt.Errorf("%s: %s", errMsg, types.ErrInvalidUserRedemptionRecord.Error())
	}
	...
	if !found {
		...
		k.Logger(ctx).Error(errMsg)
		return nil, fmt.Errorf("%s: %s", errMsg, types.ErrInvalidUserRedemptionRecord)
	}
