...
func (k msgServer) RedeemStake(goCtx context.Context, msg *types.MsgRedeemStake) (*types.MsgRedeemStakeResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("creator address is invalid: %s. err: %s", msg.Creator, err.Error())
	}
	// then make sure host zone is valid
	hostZone, found := k.GetHostZone(ctx, msg.HostZone)
	if !found {
		return nil, fmt.Errorf(types.ErrInvalidHostZone.Error(), "host zone is invalid: %s", msg.HostZone)
	}
	// first construct a user redemption record
	epochTracker, found := k.GetEpochTracker(ctx, "day")
	if !found {
		return nil, fmt.Errorf(types.ErrEpochNotFound.Error(), "epoch tracker found: %s", "day")
	}
	...
	if found {
		return nil, fmt.Errorf(recordstypes.ErrRedemptionAlreadyExists.Error(), "user already redeemed this epoch: %s", redemptionId)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("invalid receiver address (%s)", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf(types.ErrInvalidAmount.Error(), "invalid amount: %s", err.Error())
	}
	...
	if !rateIsSafe || (err != nil) {
		...
		return nil, fmt.Errorf(types.ErrRedemptionRateOutsideSafetyBounds.Error(), errMsg)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("could not parse inCoin: %s. err: %s", coinString, err.Error())
	}
	// safety checks on the coin
	// 	- Redemption amount must be positive
	if !nativeAmount.IsPositive() {
		return nil, fmt.Errorf("amount must be greater than 0. found: %d", msg.Amount)
	}
	...
	if balance.Amount.LT(sdk.NewInt(amt)) {
		return nil, fmt.Errorf("balance is lower than redemption amount. redemption amount: %d, balance %d: ", msg.Amount, balance.Amount)
	}
	...
	if !found {
		...
		return nil, fmt.Errorf(recordstypes.ErrEpochUnbondingRecordNotFound.Error(), "latest epoch unbonding record not found")
	}
	// get relevant host zone on this epoch unbonding record
	hostZoneUnbonding, found := k.RecordsKeeper.GetHostZoneUnbondingByChainId(ctx, epochUnbondingRecord.EpochNumber, hostZone.ChainId)
	if !found {
		return nil, fmt.Errorf(types.ErrInvalidHostZone.Error(), "host zone not found in unbondings: %s", hostZone.ChainId)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf(types.ErrInsufficientFunds.Error(), "couldn't send %d derivative %s tokens to module account. err: %s", msg.Amount, hostZone.HostDenom, err.Error())
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf(types.ErrIntCast.Error(), errMsg)
	}
	...
	if !success {
		...
		return nil, fmt.Errorf(types.ErrEpochNotFound.Error(), "couldn't set host zone epoch unbonding record. err: %s", err.Error())
	}
