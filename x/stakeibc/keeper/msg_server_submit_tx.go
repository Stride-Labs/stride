...
// 	// TODO set prefix dynamically, hardcoded to GAIA now
// 	address, err := bech32.ConvertAndEncode("cosmos", accAddr)
// 	if err != nil {
// 		return err
// 	}

// 	return SetDelegationAccountBalanceForDenom(k, ctx, zone, address, coin)
// }

// Simple UpdateBalances Updater using new ICQ module
func (k Keeper) UpdateUB(ctx sdk.Context, zoneInfo types.HostZone, height int64) {
	k.Logger(ctx).Info(fmt.Sprintf("\tUpdating balances on %s", zoneInfo.ChainId))
	...
	// k.InterchainQueryKeeper.QueryHostZone(ctx, zoneInfo, AccountBalanceCallback, "cosmos.staking.v1beta1.Query/DelegatorDelegations", delegationIca.Address, height)
}

// SetDelegationAccountBalanceForDenom sets the balance on an account for a given denominination.
func SetDelegationAccountBalanceForDenom(k Keeper, ctx sdk.Context, zone types.HostZone, address string, coin sdk.Coin, blockHeight int64) error {

	if address == zone.DelegationAccount.Address {
		// set balance
		da := zone.DelegationAccount
		da.UndelegatedBalance = coin.Amount.Int64()
		// only update height if this is the most updated query (ICQ msgresponses are not always FIFO)
		if blockHeight >= da.HeightLastQueriedUndelegatedBalance {
			da.HeightLastQueriedUndelegatedBalance = blockHeight
			zone.DelegationAccount = da
			k.SetHostZone(ctx, zone)
			k.Logger(ctx).Info(fmt.Sprintf("Just set UndelegatedBalance to: %d", da.DelegatedBalance))
			k.Logger(ctx).Info(fmt.Sprintf("Just set HeightLastQueriedUndelegatedBalance to: %d", blockHeight))
			ctx.EventManager().EmitEvents(sdk.Events{
				sdk.NewEvent(
					sdk.EventTypeMessage,
