...
func (s *KeeperTestSuite) TestRegisterHostZone_Success() {
	...
	// Register host zone
	_, err := s.MsgServer.RegisterHostZone(sdk.WrapSDKContext(s.Ctx), &msg)
...
func (s *KeeperTestSuite) TestRegisterHostZone_DuplicateConnectionIdInStakeibcState() {
	...
	_, err = s.MsgServer.RegisterHostZone(sdk.WrapSDKContext(s.Ctx), &newHostZoneMsg)
...
func (s *KeeperTestSuite) TestRegisterHostZone_DuplicateHostDenom() {
	...
	_, err := s.MsgServer.RegisterHostZone(sdk.WrapSDKContext(s.Ctx), &tc.validMsg)
	s.Require().NoError(err, "able to successfully register host zone once")
	...
	_, err = s.MsgServer.RegisterHostZone(sdk.WrapSDKContext(s.Ctx), &invalidMsg)
