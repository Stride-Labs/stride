...
func (k msgServer) RebalanceValidators(goCtx context.Context, msg *types.MsgRebalanceValidators) (*types.MsgRebalanceValidatorsResponse, error) {
	...
	if total_delegation.IsZero() {
		return nil, fmt.Errorf("no validator delegations found for Host Zone %s, cannot rebalance 0 delegations!: %s", hostZone.ChainId, types.ErrInvalidRequest.Error())
	}
	...
	if delegationIca == nil || delegationIca.GetAddress() == "" {
		...
		return nil, fmt.Errorf("Invalid delegation account: %s", types.ErrInvalidAddress.Error())
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to SubmitTxs for %s, %s, %s, %s: %s", connectionId, hostZone.ChainId, msgs, err.Error(), types.ErrInvalidRequest.Error())
	}
