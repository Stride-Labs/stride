...
func (k Keeper) GetValidatorDelegationAmtDifferences(ctx sdk.Context, hostZone types.HostZone) (map[string]cosmosmath.Int, error) {
	validators := hostZone.GetValidators()
	delegationDelta := make(map[string]cosmosmath.Int)
...
func (k Keeper) GetTargetValAmtsForHostZone(ctx sdk.Context, hostZone types.HostZone, finalDelegation cosmosmath.Int) (map[string]cosmosmath.Int, error) {
	// Confirm the expected delegation amount is greater than 0
	if finalDelegation == cosmosmath.ZeroInt() {
	...
	targetUnbondingsByValidator := make(map[string]cosmosmath.Int)
	totalAllocated := cosmosmath.ZeroInt()
	for i, validator := range validators {
		...
		if i == len(validators)-1 {
			...
			delegateAmt := cosmosmath.NewIntFromUint64(validator.Weight).Mul(finalDelegation).Quo(cosmosmath.NewIntFromUint64(totalWeight))
...
func (k Keeper) GetTotalValidatorDelegations(hostZone types.HostZone) cosmosmath.Int {
	validators := hostZone.GetValidators()
	total_delegation := cosmosmath.ZeroInt()
	for _, validator := range validators {
