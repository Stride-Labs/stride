...
func UndelegateCallback(k Keeper, ctx sdk.Context, packet channeltypes.Packet, ack *channeltypes.Acknowledgement, args []byte) error {
	...
	if !found {
		return fmt.Errorf("Host zone not found: %s: key not found", undelegateCallback.HostZoneId)
	}
...
func (k Keeper) GetLatestCompletionTime(ctx sdk.Context, txMsgData *sdk.TxMsgData) (*time.Time, error) {
	...
	for _, msgResponseBytes := range txMsgData.Data {
		...
		if msgResponseBytes == nil || msgResponseBytes.Data == nil {
			return nil, fmt.Errorf("msgResponseBytes or msgResponseBytes.Data is nil: %s", types.ErrTxMsgDataInvalid.Error())
		}
...
func (k Keeper) UpdateHostZoneUnbondings(
	...
	for _, epochNumber := range undelegateCallback.EpochUnbondingRecordIds {
		...
		if !success {
			...
			return 0, fmt.Errorf("couldn't set host zone epoch unbonding record. err: %s: %s", err.Error(), types.ErrEpochNotFound.Error())
		}
...
func (k Keeper) BurnTokens(ctx sdk.Context, zone types.HostZone, stTokenBurnAmount int64) error {
	...
	if err != nil {
		return fmt.Errorf("could not parse burnCoin: %s. err: %s: invalid coins", stCoinString, err.Error())
	}
	...
	if err != nil {
		...
		return fmt.Errorf("couldn't burn %d %s tokens in module account. err: %s: %s", stTokenBurnAmount, stCoinDenom, err.Error(), types.ErrInsufficientFunds.Error())
	}
