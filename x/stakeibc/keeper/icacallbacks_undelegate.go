...
func (k Keeper) UpdateDelegationBalances(ctx sdk.Context, hostZone types.HostZone, undelegateCallback types.UndelegateCallback) (err error) {
	...
	for _, undelegation := range undelegateCallback.SplitDelegations {
		hostZone, err = k.UpdateValidatorDelegation(ctx, hostZone, undelegation.Validator, undelegation.Amount.Neg(), ICACallbackID_Undelegate)
		if err != nil {
			return errorsmod.Wrapf(err, "Failed to remove delegation after unbonding")
		}
	}
