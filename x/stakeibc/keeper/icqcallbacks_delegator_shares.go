...
func DelegatorSharesCallback(k Keeper, ctx sdk.Context, args []byte, query icqtypes.Query) error {
	...
	if err != nil {
		return fmt.Errorf("unable to unmarshal query response into Delegation type, err: %s: %s", err.Error(), types.ErrMarshalFailure.Error())
	}
	...
	if !found {
		return fmt.Errorf("no registered validator for address (%s): %s", queriedDelgation.ValidatorAddress, types.ErrValidatorNotFound.Error())
	}

	// Get the validator's internal exchange rate, aborting if it hasn't been updated this epoch
	strideEpochTracker, found := k.GetEpochTracker(ctx, epochtypes.STRIDE_EPOCH)
	if !found {
		return fmt.Errorf("unable to get epoch tracker for epoch (%s): %s", epochtypes.STRIDE_EPOCH, types.ErrNotFound.Error())
	}
	if validator.InternalExchangeRate.EpochNumber != strideEpochTracker.EpochNumber {
		return fmt.Errorf("validator (%s) internal exchange rate has not been updated this epoch (epoch #%d): %s", validator.Address, strideEpochTracker.EpochNumber, types.ErrInvalidRequest.Error())
	}
	...
	if delegatedTokens.GT(validator.DelegationAmt) {
		return fmt.Errorf("Validator (%s) tokens returned from query is greater than the DelegationAmt: %s", validator.Address, types.ErrInvalidRequest.Error())
	}
	...
	if slashPct.GT(slashThresholdDecimal) {
		return fmt.Errorf("Validator slashed but ABORTING update, slash (%v) is greater than safety threshold (%v): %s", slashPct, slashThresholdDecimal, types.ErrSlashExceedsSafetyThreshold.Error())
	}

	// Update the validator weight and delegation reflect to reflect the slash
	weight, err := cast.ToInt64E(validator.Weight)
	if err != nil {
		return fmt.Errorf("unable to convert validator weight to int64, err: %s: %s", err.Error(), types.ErrIntCast.Error())
	}
