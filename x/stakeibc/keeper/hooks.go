...
func (k Keeper) BeforeEpochStart(ctx sdk.Context, epochInfo epochstypes.EpochInfo) {
	...
	if epochInfo.Identifier == epochstypes.DAY_EPOCH {
		for _, hostZone := range hostZones {
			...
			k.InitiateAllHostZoneUnbondings(ctx, epochNumber, hostZone)
			// Check previous epochs to see if unbondings finished, and sweep the tokens if so
			k.SweepAllUnbondedTokens(ctx, hostZone)
		}
	...
	if epochInfo.Identifier == epochstypes.STRIDE_EPOCH {
		...
		for _, hostZone := range hostZones {
			if epochNumber%redemptionRateInterval == 0 {
				k.UpdateRedemptionRates(ctx, depositRecords, hostZone)
			}
			...
			if epochNumber%depositInterval == 0 {
				k.TransferExistingDepositsToHostZones(ctx, epochNumber, depositRecords, hostZone)
			}

			// Delegate tokens from the delegation account
			if epochNumber%delegationInterval == 0 {
				k.StakeExistingDepositsOnHostZones(ctx, epochNumber, depositRecords, hostZone)
			}
			...
			if epochNumber%reinvestInterval == 0 { // allow a few blocks from UpdateUndelegatedBal to avoid conflicts
				k.ReinvestRewards(ctx, hostZone)
			}
		}
	}
}
...
func (k Keeper) UpdateRedemptionRates(ctx sdk.Context, depositRecords []recordstypes.DepositRecord, hostZone types.HostZone) {
...
func (k Keeper) ReinvestRewards(ctx sdk.Context, hostZone types.HostZone) {
