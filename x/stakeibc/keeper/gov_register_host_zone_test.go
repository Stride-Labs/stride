...
type RegisterHostZoneTestCase struct {
	validMsg                   stakeibctypes.RegisterHostZoneProposal
...
func (s *KeeperTestSuite) SetupRegisterHostZone() RegisterHostZoneTestCase {
	...
	defaultMsg := stakeibctypes.RegisterHostZoneProposal{
...
func (s *KeeperTestSuite) createNewHostZoneMessage(chainID string, denom string, prefix string) stakeibctypes.RegisterHostZoneProposal {
	...
	return stakeibctypes.RegisterHostZoneProposal{
...
func (s *KeeperTestSuite) TestRegisterHostZone_Success() {
	...
	// Register host zone
	err := s.App.StakeibcKeeper.RegisterHostZoneProposal(s.Ctx, &msg)
...
func (s *KeeperTestSuite) TestRegisterHostZone_DuplicateConnectionIdInStakeibcState() {
	...
	err = s.App.StakeibcKeeper.RegisterHostZoneProposal(s.Ctx, &newHostZoneMsg)
...
func (s *KeeperTestSuite) TestRegisterHostZone_DuplicateHostDenom() {
	...
	err := s.App.StakeibcKeeper.RegisterHostZoneProposal(s.Ctx, &tc.validMsg)
	s.Require().NoError(err, "able to successfully register host zone once")
	...
	err = s.App.StakeibcKeeper.RegisterHostZoneProposal(s.Ctx, &invalidMsg)
