...
func (k Keeper) GetStrideEpochElapsedShare(ctx sdk.Context) (sdk.Dec, error) {
	...
	if !found {
		...
		k.Logger(ctx).Error(errMsg)
		return sdk.ZeroDec(), fmt.Errorf("%s: %s", errMsg, types.ErrNotFound.Error())
	}
	...
	if err != nil {
		...
		k.Logger(ctx).Error(errMsg)
		return sdk.ZeroDec(), fmt.Errorf("%s: %s", errMsg, types.ErrIntCast.Error())
	}
	...
	if currBlockTime < epochStartTime || currBlockTime > epochEndTime {
		...
		k.Logger(ctx).Error(errMsg)
		return sdk.ZeroDec(), fmt.Errorf("%s: %s", errMsg, types.ErrInvalidEpoch.Error())
	}
...
func (k Keeper) GetICATimeoutNanos(ctx sdk.Context, epochType string) (uint64, error) {
	...
	if bufferSize > epochTracker.Duration {
		...
		return 0, fmt.Errorf("Invalid buffer size %d: %s", bufferSize, types.ErrInvalidRequest.Error())
	}
