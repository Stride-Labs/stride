...
func (k msgServer) LiquidStake(goCtx context.Context, msg *types.MsgLiquidStake) (*types.MsgLiquidStakeResponse, error) {
	...
	if balance.IsLT(inCoin) {
		...
		return nil, fmt.Errorf("balance is lower than staking amount. staking amount: %v, balance: %v: %s", msg.Amount, balance.Amount, types.ErrInsufficientFunds.Error())
	}
	...
	if !found {
		...
		return nil, fmt.Errorf("no epoch number for epoch (%s): %s", epochtypes.STRIDE_EPOCH, types.ErrNotFound.Error())
	}
	...
	if !found {
		...
		return nil, fmt.Errorf("no deposit record for epoch (%d): %s", strideEpochTracker.EpochNumber, types.ErrNotFound.Error())
	}
...
func (k msgServer) MintStAssetAndTransfer(ctx sdk.Context, sender sdk.AccAddress, amount sdkmath.Int, denom string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to parse coins %s: %s", coinString, types.ErrInvalidRequest.Error())
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to mint coins: %s", types.ErrInvalidRequest.Error())
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to send %s from module to account: %s", stCoins.GetDenomByIndex(0), types.ErrInvalidRequest.Error())
	}
