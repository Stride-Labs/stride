...
func (k msgServer) LiquidStake(goCtx context.Context, msg *types.MsgLiquidStake) (*types.MsgLiquidStakeResponse, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf(types.ErrInvalidHostZone.Error(), "no host zone found for denom (%s)", msg.HostDenom)
	}
	...
	if balance.IsLT(inCoin) {
		...
		return nil, fmt.Errorf("balance is lower than staking amount. staking amount: %d, balance: %d", msg.Amount, balance.Amount.Int64())
	}
	...
	if !isIbcToken {
		...
		return nil, fmt.Errorf(types.ErrInvalidToken.Error(), "denom is not an IBC token (%s)", ibcDenom)
	}
	...
	if err != nil {
		k.Logger(ctx).Error("failed to send tokens from Account to Module")
		return nil, fmt.Errorf(err.Error(), "failed to mint %s stAssets to user", msg.HostDenom)
	}
	...
	if !found {
		...
		return nil, fmt.Errorf("no epoch number for epoch (%s)", epochtypes.STRIDE_EPOCH)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf(err.Error(), "failed to convert msg.Amount to int64")
	}
...
func (k msgServer) MintStAsset(ctx sdk.Context, sender sdk.AccAddress, amount uint64, denom string) error {
	...
	if err != nil {
		...
		return fmt.Errorf(err.Error(), "failed to convert amount to int64")
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to parse coins %s", coinString)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to mint coins")
	}
	...
	if err != nil {
		...
		return fmt.Errorf("Failed to send %s from module to account", stCoins.GetDenomByIndex(0))
	}
