...
type DeleteValidatorTestCase struct {
	...
	validMsgs         []stakeibctypes.DeleteValidatorsProposal
}
	...
	validMsgs := []stakeibctypes.DeleteValidatorsProposal{
		{
			...
			Description: "",
			HostZone:    "GAIA",
			ValAddrs:    []string{"stride_VAL1"},
		},
		{
			Title:       "",
			Description: "",
			HostZone:    "GAIA",
			ValAddrs:    []string{"stride_VAL2"},
		},
	}
...
func (s *KeeperTestSuite) TestDeleteValidator_Successful() {
	...
	err := s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &tc.validMsgs[0])
	...
	err = s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &tc.validMsgs[1])
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestDeleteValidator_HostZoneNotFound() {
	...
	err := s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &badHostZoneMsg)
	errMsg := "HostZone (gaia) not found: host zone not found"
...
func (s *KeeperTestSuite) TestDeleteValidator_AddressNotFound() {
	...
	badAddressMsg.ValAddrs = []string{"stride_VAL5"}
	err := s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &badAddressMsg)

	errMsg := "Validator address (stride_VAL5) not found on host zone (GAIA): validator not found"
	s.Require().EqualError(err, errMsg)
}
...
func (s *KeeperTestSuite) TestDeleteValidator_NonZeroDelegation() {
	...
	err := s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &tc.validMsgs[0])
	errMsg := "Validator (stride_VAL1) has non-zero delegation (1) or weight (0)"
	s.Require().EqualError(err, errMsg)
}
...
func (s *KeeperTestSuite) TestDeleteValidator_NonZeroWeight() {
	...
	err := s.App.StakeibcKeeper.DeleteValidatorsProposal(s.Ctx, &tc.validMsgs[0])
	errMsg := "Validator (stride_VAL1) has non-zero delegation (0) or weight (1)"
	s.Require().EqualError(err, errMsg)
}
