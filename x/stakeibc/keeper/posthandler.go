...
func (stakeIbcDec StakeIbcPostDecorator) PostHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, success bool,  next sdk.PostHandler) (sdk.Context, error) {
	...
	for _, hz := range hostzones {
		reComputeRedemptionRate := stakeIbcDec.StakeIbcKeeper.ComputeRedemptionRate(cacheCtx, hz)
		stakeIbcDec.StakeIbcKeeper.CompareRedemptionRate(cacheCtx, reComputeRedemptionRate, hz)
	}
	write()
