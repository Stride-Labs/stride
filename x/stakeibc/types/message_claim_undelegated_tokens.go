...
func (msg *MsgClaimUndelegatedTokens) ValidateBasic() error {
	...
	if err != nil {
		return fmt.Errorf("invalid sender address (%s): %s", err, ErrInvalidAddress.Error())
	}
	...
	if !(msg.Epoch < (1<<63 - 1)) {
		return fmt.Errorf("epoch must be less than math.MaxInt64 %d: %s", 1<<63-1, ErrInvalidAmount.Error())
	}
	return nil
}
