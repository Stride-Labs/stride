...
func (msg *MsgUpdateValidatorSharesExchRate) ValidateBasic() error {
	...
	if len(msg.Valoper) == 0 {
		return fmt.Errorf("valoper is required: %s", ErrInvalidRequest.Error())
	}
	if !strings.Contains(msg.Valoper, "valoper") {
		return fmt.Errorf("validator operator address must contrain 'valoper': %s", ErrInvalidRequest.Error())
	}
