...
func (msg *MsgUpdateValidatorSharesExchRate) ValidateBasic() error {
	...
	if len(msg.ChainId) == 0 {
		return sdkerrors.Wrapf(legacysdkerrors.ErrInvalidRequest, "chainid is required")
	}
	// basic checks on host zone
	if len(msg.Valoper) == 0 {
		return sdkerrors.Wrapf(legacysdkerrors.ErrInvalidRequest, "valoper is required")
	}
	if !strings.Contains(msg.Valoper, "valoper") {
		return sdkerrors.Wrapf(legacysdkerrors.ErrInvalidRequest, "validator operator address must contrain 'valoper'")
	}
