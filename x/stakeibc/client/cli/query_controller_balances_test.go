...
func networkWithControllerBalancesObjects(t *testing.T, n int) (*network.Network, []types.ControllerBalances) {
	...
	return network.New(t, cfg), state.ControllerBalancesList
}

func TestShowControllerBalances(t *testing.T) {
	net, objs := networkWithControllerBalancesObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetControllerBalancesResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.ControllerBalances)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListControllerBalances(t *testing.T) {
	net, objs := networkWithControllerBalancesObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListControllerBalances(), args)
			require.NoError(t, err)
			var resp types.QueryAllControllerBalancesResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
