...
func networkWithEpochTrackerObjects(t *testing.T, n int) (*network.Network, []types.EpochTracker) {
	t.Helper()
	...
	return network.New(t, cfg), state.EpochTrackerList
}

func TestShowEpochTracker(t *testing.T) {
	net, objs := networkWithEpochTrackerObjects(t, 2)
...
func TestListEpochTracker(t *testing.T) {
	net, objs := networkWithEpochTrackerObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListEpochTracker(), args)
			require.NoError(t, err)
			var resp types.QueryAllEpochTrackerResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
