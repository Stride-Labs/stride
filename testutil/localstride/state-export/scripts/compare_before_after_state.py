...
def _return_equal_unequal_paths_recursively(s1, s2, prefix):
    if (type(s1) is not dict) or (type(s2) is not dict):
        if s1 == s2:
            return [prefix], []
        else:
            if (
                ...
                and (type(s1) is list)
                and (len(s1) == len(s2))
                and (prefix not in EXPECTED_DIFFS)
            ):
    ...
    for joint_key in inter_keys:
        sub_equal, sub_unequal = _return_equal_unequal_paths_recursively(
            s1[joint_key], s2[joint_key], prefix + f'/{joint_key}'
        )
