...
$STRIDE_MAIN_CMD tx gov submit-legacy-proposal software-upgrade $upgrade_name \
    ...
    --upgrade-height $upgrade_height --from val --upgrade-info "test" --no-validate -y | TRIM_TX
...
sleep 5 
printf "\nDEPOSIT\n"
$STRIDE_MAIN_CMD tx gov deposit $proposal_id 2000000001ustrd --from val -y | TRIM_TX
