# Validator Weights (v33 Upgrade)

This directory contains the data and scripts used to compute the validator weight
updates for the v33 upgrade handler (Q2 2026 targets).

Unlike v32 (which touched all 15 host zones), this round only updates the 10 chains
with a raw spreadsheet export in `target/raw/`. Host zones without a CSV
(comdex-1, evmos_9001-2, sommelier-3, stargaze-1, umee-1) are intentionally left
untouched by the upgrade handler.
...
3. **Identify new validators** — Any validator in the target that doesn't exist on-chain
   ...
   the existing on-chain naming convention. Where an operator migrated to a new valoper
   address and the old entry (kept at weight 0) still holds the conventional name, the
   new entry gets a `2` suffix (`twinstake2`, `bitgobytwinstake2`, `hextrust2`).
...
4. **Build the full validator set** — The union of current and target validators is written
   ...
   the target. Restricted to the 10 in-scope chains.
...
6. **Generate Go code** — `scripts/generate_validators_go.py` reads `new_validators.csv`,
   `all_validators.csv`, and `current_weights.csv` to produce `../weights.go`, which
   contains the `OldValidators` (test seed data), `NewValidators`, and `TargetWeights`
   data used by the upgrade handler.

## Files

| File | Description |
|------|-------------|
| `current_weights.csv` | Current on-chain validator weights fetched from the Stride API (all 15 zones) |
| `target_weights.csv` | Target weights (basis points) for validators with non-zero allocation |
| `all_validators.csv` | Union of current + target validators on in-scope chains; 0 weight means the validator should be zeroed out |
...
| `scripts/generate_validators_go.py` | Generates `../weights.go` from the intermediate CSVs |
...
# Step 3: Generate weights.go for the upgrade handler
