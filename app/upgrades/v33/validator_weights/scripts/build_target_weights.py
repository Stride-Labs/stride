...
Only chains with a raw target CSV are included — host zones without a CSV are
intentionally left untouched by the v33 upgrade.
...
def parse_chain_id_from_filename(filename: str) -> str:
    return filename.replace("Stride Delegations Q2 2026 - ", "").replace(".csv", "")
...
def normalize_weights(validators: list[dict]) -> list[dict]:
    """Adjusts weights so they sum to exactly TARGET_SUM by tweaking the largest validator."""
...
def load_current_weights(target_chain_ids: set[str]) -> dict[str, list[dict]]:
    """Returns {chain_id: [{address, name, weight}, ...]} from current_weights.csv.

    Restricted to chains present in the target — other host zones are out of
    scope for this upgrade and must not appear in the generated Go data.
    """
    result: dict[str, list[dict]] = {}
...
def main() -> int:
    ...
    print("Loading current weights (target chains only)...")
    current_by_chain = load_current_weights(set(target_by_chain))
