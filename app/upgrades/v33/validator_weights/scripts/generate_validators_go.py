"""
Generates weights.go from new_validators.csv and all_validators.csv.

Output: app/upgrades/v33/weights.go
"""
...
OUTPUT_PATH = WEIGHTS_DIR.parent / "weights.go"
...
def build_go_source(
    current_vals: dict[str, list[dict]],
    new_vals: dict[str, list[dict]],
    all_vals: dict[str, list[dict]],
) -> str:
    lines = []
    lines.append("package v33")
    lines.append("")
    ...
    lines.append("var OldValidators = map[string][]ValidatorConfig{")
    for chain_id in sorted(current_vals):
        lines.append(f'\t"{chain_id}": {{')
        for v in current_vals[chain_id]:
            w = int(v["current_weight"])
            lines.append(
                f'\t\t{{Address: "{v["validator_address"]}", Name: "{v["validator_name"]}", Weight: {w}}},'
            )
        lines.append("\t},")
    lines.append("}")
    lines.append("")
    ...
    for chain_id in sorted(new_vals):
        ...
        for v in new_vals[chain_id]:
            lines.append(
                f'\t\t{{Address: "{v["validator_address"]}", Name: "{v["validator_name"]}"}},'
            )
        lines.append("\t},")
    lines.append("}")
    lines.append("")
...
def main() -> int:
    ...
    print(
        f"Found {sum(len(v) for v in all_vals.values())} total validators across {len(all_vals)} chains"
    )

    # Only include current validators for chains being updated in this upgrade
    print(f"Loading {CURRENT_WEIGHTS_PATH} ...")
    current_vals = {
        chain_id: rows
        for chain_id, rows in load_csv_by_chain(CURRENT_WEIGHTS_PATH).items()
        if chain_id in all_vals
    }
    print(
        f"Found {sum(len(v) for v in current_vals.values())} current validators across {len(current_vals)} chains"
    )

    print(f"Loading {NEW_VALIDATORS_PATH} ...")
    new_vals = load_csv_by_chain(NEW_VALIDATORS_PATH)
    print(
        f"Found {sum(len(v) for v in new_vals.values())} new validators across {len(new_vals)} chains"
    )

    source = build_go_source(current_vals, new_vals, all_vals)
