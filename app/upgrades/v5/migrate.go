...
func migrateStakeibcParams(store sdk.KVStore, cdc codec.Codec) error {
	// paramsStore := prefix.NewStore(store, []byte(claimtypes.ParamsKey))
	oldBz := store.Get([]byte(stakeibctypes.ParamsKey))
	var oldProp stakeibcv1types.Params
	err := cdc.UnmarshalJSON(oldBz, &oldProp)
	if err != nil {
		return err
	}
	newProp := convertToNewStakeIbcParams(oldProp)
	newBz, err := cdc.MarshalJSON(&newProp)
	if err != nil {
		return err
	}
	// Set new value on store.
	store.Set([]byte(stakeibctypes.ParamsKey), newBz)
	return nil
}
...
func MigrateStore(ctx sdk.Context, claimStoreKey storetypes.StoreKey, recordStoreKey storetypes.StoreKey, stakeibcStoreKey storetypes.StoreKey, cdc codec.Codec) error {
	...
	err = migrateStakeibcParams(stakeibcStore, cdc)
	if err != nil {
		return err
	}
