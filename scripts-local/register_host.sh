...
echo $STRIDE_ADMIN_MNEMONIC | $STRIDE_CMD keys add $STRIDE_ADMIN_ACCT --recover --keyring-backend=test &> /dev/null

echo "Funding stride admin account..."
STRIDE_ADMIN_ADDRESS=$($STRIDE_CMD keys show $STRIDE_ADMIN_ACCT -a) 
$STRIDE_CMD tx bank send $STRIDE_VAL_ADDR $STRIDE_ADMIN_ADDRESS 10000000ustrd --from $STRIDE_VAL_ACCT -y --chain-id $STRIDE_CHAIN
sleep 10
...
$STRIDE_CMD tx stakeibc register-host-zone \
    ...
    --home $STATE/stride --from $STRIDE_ADMIN_ACCT --gas 1000000 --chain-id $STRIDE_CHAIN -y
...
CSLEEP 10
$STRIDE_CMD tx stakeibc add-validator GAIA gval1 $GAIA_DELEGATE_VAL 10 5 --from $STRIDE_ADMIN_ACCT --chain-id $STRIDE_CHAIN -y
CSLEEP 30
$STRIDE_CMD tx stakeibc add-validator GAIA gval2 $GAIA_DELEGATE_VAL_2 10 10 --from $STRIDE_ADMIN_ACCT --chain-id $STRIDE_CHAIN -y
CSLEEP 30
