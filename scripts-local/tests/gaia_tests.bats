...
@test "[INTEGRATION-BASIC-GAIA] tokens on GAIA were staked" {
  ...
  # let the IBC calls
  WAIT_FOR_BLOCK $STRIDE_LOGS
  WAIT_FOR_STRING $STRIDE_LOGS 'DelegateCallback hostZoneId:"GAIA" depositRecordId'
  # check staked tokens
...
#   # do IBC transfer
#   $STRIDE_CMD tx ibc-transfer transfer transfer channel-1 $JUNO_ADDRESS 3000ustrd --from val1 --chain-id STRIDE -y --keyring-backend test &
