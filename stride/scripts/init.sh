...
for i in ${!STRIDE_CHAINS[@]}; do
    ...
    # now we grab the relevatn node id
    ...
    then
        ...
        # yes | $main_cmd gentx $val_acct 1000000000ustrd --chain-id $chain_name --keyring-backend test
        cp ./${STATE}/${chain_name}/config/gentx/*.json ./${STATE}/${main_chain}/config/gentx/
    fi
done

# strided --home state/STRIDE_1 add-genesis-account ${VAL_ADDR} 500000000000ustrd
# strided --home state/STRIDE_1 collect-gentxs 
