...
func networkWithHostZoneObjects(t *testing.T) (*network.Network, types.HostZone) {
	t.Helper()
	...
	state.HostZone = hostZone
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), *state.HostZone
}

func TestShowHostZone(t *testing.T) {
	net, obj := networkWithHostZoneObjects(t)
