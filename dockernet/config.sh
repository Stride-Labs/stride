...
if [[ "${ALL_HOST_CHAINS:-false}" == "true" ]]; then 
  HOST_CHAINS=(GAIA OSMO HOST COMDEX)
...
IBC_COMDEX_DENOM=$IBC_COMDEX_CHANNEL_3_DENOM
...
COMDEX_NUM_NODES=1
COMDEX_BINARY="$DOCKERNET_HOME/../build/comdex"
...
COMDEX_MAIN_CMD="$COMDEX_BINARY --home $DOCKERNET_HOME/state/${COMDEX_NODE_PREFIX}1"
COMDEX_RECEIVER_ADDRESS='comdex1e40sj8wpd8yfjw56cs5kn8ucdeqz2vsqjpjk95'
...
RELAYER_ACCTS=($RELAYER_GAIA_ACCT $RELAYER_JUNO_ACCT $RELAYER_OSMO_ACCT $RELAYER_STARS_ACCT $RELAYER_HOST_ACCT $RELAYER_COMDEX_ACCT)
