...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] autopilot redeem stake" {
  ...
  redemption_record_amount=$($STRIDE_MAIN_CMD q records list-user-redemption-record  | grep -Fiw 'native_amount' | head -n 1 | grep -o -E '[0-9]+')
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] redemption and undelegation on $CHAIN_NAME" {
  ...
  num_records=$($STRIDE_MAIN_CMD q records list-user-redemption-record | grep -c "native_amount")
